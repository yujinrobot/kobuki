...
KeyOpCore::KeyOpCore() :
	...
	angular_vel_max(1.2),
	tilt_angle_step_(0.5),
	tilt_angle_min_(-31.0),
	tilt_angle_max_(31.0),
	mode_("full"),
...
void KeyOpCore::init() {
	...
	**********************/
	...
	nh.getParam("tilt_angle_step", tilt_angle_step_);
	nh.getParam("tilt_angle_min" , tilt_angle_min_ );
	nh.getParam("tilt_angle_max" , tilt_angle_max_ );
	...
	ROS_INFO_STREAM("KeyOpCore : using tilt angle step [" << tilt_angle_step_ << "].");
	ROS_INFO_STREAM("KeyOpCore : using tilt angle min  [" << tilt_angle_min_  << "].");
	ROS_INFO_STREAM("KeyOpCore : using tilt angle max  [" << tilt_angle_max_  << "].");
	...
	**********************/
	...
	kinect_tilt_angle_publisher = nh.advertise<std_msgs::Float64>("kinect_tilt_angle",1);
	...
	** Kinect's tilt angle
	**********************/
	tilt_angle_.data = 0.0;

	if( mode_ == "simple" ) return;
...
void KeyOpCore::keyboardInputLoop() {
	...
	puts("A/Z : kinect tilt angle incr/decr.");
...
void KeyOpCore::processKeyboardInput(char c) {
	...
	switch(c) {
	    ...
	    case kobuki_comms::KeyboardInput::KeyCode_Space: {
            ...
            case kobuki_comms::KeyboardInput::KeyCode_PageUp: {
                incrementTiltAngle();
                break;
            }
            case kobuki_comms::KeyboardInput::KeyCode_PageDown: {
                decrementTiltAngle();
                break;
            }
	    case 'q': {
...
/**
 * @brief If not already maxxed, increment the kinect's tilt angle
 */
void KeyOpCore::incrementTiltAngle()
{
  if ( power_status )
  {
    if ( tilt_angle_.data < tilt_angle_max_ )
    {
      tilt_angle_.data += tilt_angle_step_;
      ROS_INFO_STREAM("KeyOp: tilt angle incremented [" << tilt_angle_.data << "].");
      kinect_tilt_angle_publisher.publish(tilt_angle_);
    }
    else
    {
      ROS_INFO_STREAM("KeyOp: maximum tilt angle reached [" << tilt_angle_.data << "].");
    }
}
  else
  {
...
/**
 * @brief If not already minned, decrement the kinect's tilt angle
 */
void KeyOpCore::decrementTiltAngle()
{
  if ( power_status )
  {
    if ( tilt_angle_.data > tilt_angle_min_ )
    {
      tilt_angle_.data -= tilt_angle_step_;
      ROS_INFO_STREAM("KeyOp: tilt angle decremented [" << tilt_angle_.data << "].");
      kinect_tilt_angle_publisher.publish(tilt_angle_);
    }
    else
    {
      ROS_INFO_STREAM("KeyOp: minimum tilt angle reached [" << tilt_angle_.data << "].");
    }
  }
  else
  {
