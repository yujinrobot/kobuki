                            ...
                            const kobuki_msgs::DockInfraRedConstPtr& ir)
{
  ...
  if( as_.isActive() ) {
    if ( dock_.getState() == RobotDockingState::DONE ) {
      ...
      ROS_INFO_STREAM( "[" << name_ << "]: Lost: did not detect dock IR lights.");
      result_.text = "Lost: did not detect dock IR lights.";
      as_.setAborted( result_, "Aborted: cannot detect dock.");
