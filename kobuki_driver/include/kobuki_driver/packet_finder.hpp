...
public:
	...
	packetFinder( const unsigned char putOneByteStx, const unsigned char putOneByteEtx,
				  unsigned int sizeLengthField, unsigned int sizeMaxPayload, unsigned int sizeChecksumField,
				  bool variableSizePayload )
	:
	size_stx(1), size_etx(1),
	size_length_field(sizeLengthField),
	variable_size_payload( variableSizePayload ),
	size_max_payload(sizeMaxPayload),
	size_payload( variable_size_payload ? 0 : sizeMaxPayload ),
	size_checksum_field(sizeChecksumField),
	STX(size_stx,putOneByteStx),
	ETX(size_etx,putOneByteEtx),
	buffer(size_stx, size_stx+size_length_field+size_max_payload+size_checksum_field+size_etx ),
	state(waitingForStx),
	verbose(false)
	{
		clear();
	...
	packetFinder( const BufferType & putStx, const BufferType & putEtx,
				  unsigned int sizeLengthField, unsigned int sizeMaxPayload, unsigned int sizeChecksumField,
				  bool variableSizePayload )
	:
	size_stx(putStx.size()), size_etx(putEtx.size()),
	size_length_field(sizeLengthField),
	variable_size_payload( variableSizePayload ),
	size_max_payload(sizeMaxPayload),
	size_payload( variable_size_payload ? 0 : sizeMaxPayload ),
...
protected:
	...
	bool waitForPayloadAndEtx( const unsigned char * incoming, unsigned int numberOfIncoming, bool & foundPacket  )
	{
		...
		{
			if( verbose ) std::cout << "Start check etx " << std::endl;
			foundPacket = true;

			for( unsigned int i=(size_stx+size_length_field+size_payload+size_checksum_field);
					i<(size_stx+size_length_field+size_payload+size_checksum_field+size_etx);
					i++ )
			{
				if( buffer[i] !=  ETX[i] )
				{
			...
			if( verbose ) std::cout << "End of checking etx " << std::endl;
			return true;
		}
	}
};
