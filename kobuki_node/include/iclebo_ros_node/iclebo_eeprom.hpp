#ifndef __ICLEBO_EEPROM_DATA_HPP__
#define __ICLEBO_EEPROM_DATA_HPP__
...
public:
	...
	bool serialise( ecl::PushAndPop<unsigned char> & byteStream )
	{
		if(!(byteStream.size()>0)) { 
			ROS_WARN_STREAM("iclebo_ros_node: iclebo_eeprom: serialise failed. empty byte stream."); 
			return false; 
		}
	...
	{
		if(!(byteStream.size()>0)) { 
			ROS_WARN_STREAM("iclebo_ros_node: iclebo_eeprom: deserialise failed. empty byte stream."); 
			return false; 
		}
...
#endif /* __ICLEBO_EEPROM_DATA_HPP__ */
